...
def perform_yolo_detection(img):
    ...
    for i in indexes.flatten():
        ...
        # Estimate distance (assume fixed focal length and known object width)
        focal_length = 700  # Example focal length in pixels (adjust based on camera calibration)
        known_width = 0.5  # Example known width of object in meters (adjust based on real objects)
        distance = (known_width * focal_length) / w
        distance_text = f"{distance:.2f} m"

        # Draw distance text
        cv2.putText(img, distance_text, (x, y - 25), font, 0.5, (255, 255, 255), 1)

        # Announce detected object and distance
        announcement = f"Detected {label} at {distance:.2f} meters"
